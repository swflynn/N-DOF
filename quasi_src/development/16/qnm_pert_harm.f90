! 10/8/17
! We believe we corrected the transformations, now need to test harmonic pertebation
! This code replaces the potential difference with a harmonic potential*0.02
...
! This last bit is testing can be removed
...
!=================These tests were to see if our transfomrations are correct===============!
!==============================Remove once done testing====================================!
! I am playing around here, try setting the dif potential as just harmonic 1-6
! Then for the 3 remaining basis set it to be 0.1*harm
potdif = 0d0
      DO j=7,Dim   ! check code to see if we can get correct result, first 6 are set to 500wavenumbers
      potdif = potdif+ pert*(0.5*omega(j)*y(j)**2)      ! if 0.5 = 1, get correct result off by factor of 2!
      END DO
...
!===========================================================================================!
        DO j=1,Jmax-1  !VM corrected: not really a mistake         
           DO j1=j+1,Jmax 
              C(j,j1) = C(j1,j) 
           END DO
        END DO
...
!              C(j,j)=C(j,j)+omega(k)*(v(j,k)+0.5)
              C(j,j)=C(j,j)+omega(k)*(v(k,j)+0.5)    !VM corrected
            END DO 
        END DO 
...
!        WRITE(80,*) i, C !Writes entire matrix out, want to change to column
