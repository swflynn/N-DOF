! 6-16-17 -Shane
! Re-Define Hermite Polynomial Coeficients according to write-up
! So herm is now the modified hermite polynomial
! The HO has been redifined such that we no longer need coeficients
...
INTEGER, PARAMETER :: deg = 1      ! Highest Degree Polynomial
...
DOUBLE PRECISION :: B
DOUBLE PRECISION, ALLOCATABLE :: scrambled_u(:), scrambled_z(:), herm(:,:), A(:,:,:), U(:,:) 
INTEGER :: i, j, j1, k, m, l, n
INTEGER Nsobol
...
ALLOCATE(scrambled_u(d), scrambled_z(d), herm(deg,d), A(deg,deg,d))
...
DO i = 1, Nsobol              
  ...
  herm(1,:) = SQRT(2.0)*scrambled_z(:)       
  DO j = 2,deg      
    herm(j,:) = (SQRT(2./j)*scrambled_z(:)*herm(j-1,:)) - (SQRT(j/(j-1d0))*herm(j-2,:))
  END DO
  WRITE(*,*) 'Herm test'                                    ! Make sure we have proper hermite evaluation
  WRITE(*,*) herm                                           ! remove once done testing
  WRITE(*,*) 'Herm test herm(0,:)'                                    ! Make sure we have proper hermite evaluation
  WRITE(*,*) herm(0,:)                                           ! remove once done testing
!=================================Evaluate Herm * Herm =================================!
...
DO i =1,Jmax
  WRITE(80,*) U(1:Jmax,i) !Writes entire matrix out
!  WRITE(80,*) U(:,i)
END DO
CLOSE(UNIT=80)
