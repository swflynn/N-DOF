...
!==============================================================================!
!    Discussion:
!       Fortran 90 implementation of quasi Monte Carlo Integration for computing
!       the Fundamental Frequencies of a single water atom within a cluster. The 
!       code reads in an xyz file for a water cluster, computes the Hessian, 
!       and Constructs a PE difference matrix between the HA and the actual PES.
!       TIP4P and MBPOL surfaces available. 
!       qMC using Scrambled Sobol Points are implemented see MCQMC for ssobol.f
!       Matrix diagonalization and eigen sovler with LaPack.
!       Paralel implementation for computing the PE difference matrix using OMP
!    Modified:
!       6 March 2018
!    Author:
!       Shane Flynn
...
!       vmax   => 
!==============================================================================!
integer :: Natoms, Dim, Jmax, Vtot, Vmax(9)
...
double precision :: Hess(Dim1,Dim1), omega(Dim1), U(Dim1,Dim1)
...
!==============================================================================!
implicit none
integer :: j,vv(Dim1),k,v1,v2,v3,v4,v5,v6,v7,v8,v9
j=0
if(Dim1>9) stop 'Spatial_Dim>9, check permutation subroutine'
do v1=0,Vmax(1)
    vv(1)=v1
    if(Dim1>= 2) then
...
Jmax = j                
write(*,*) 'Jmax = ', Jmax
ALLOCATE(v(Dim1,Jmax))
j=0
if(Dim1>9) stop 'Spatial_Dim>9'
do v1=0,Vmax(1)
vv(1)=v1
if(Dim1>= 2) then
  do v2=0,min(Vtot-vv(1),Vmax(2))
...
CALL permutation()
Jmax2=(Jmax*(Jmax+1))/2
ALLOCATE(Umat(Jmax2), U1mat(Jmax2), C(Jmax,Jmax), eigenvalue(Jmax), y(Dim1), &
        herm(0:Vtot,Dim1), A(0:Vtot,0:Vtot,Dim1))
...
!==============================================================================!
OPEN(UNIT=83, FILE='fundamentals.dat')
CALL INSSOBL(FLAG,Dim1,Nsobol,TAUS,y,max_seq,IFLAG)
...
!==============================================================================!
        ...
        E3 = 0d0
        do m = 1,Jmax
            if(all(v(:,m)==(/0,0,0,0,0,0,0,0,0/))) then
                j2=maxloc(abs(C(m,:)))
                E0 = eigenvalue(j2)*autocm
            else if(all(v(:,m)==(/0,0,0,0,0,0,1,0,0/))) then
                j2=maxloc(abs(C(m,:)))
                E1 = eigenvalue(j2)*autocm              
            else if(all(v(:,m)==(/0,0,0,0,0,0,0,1,0/))) then
                j2=maxloc(abs(C(m,:)))
                E2 = eigenvalue(j2)*autocm              
            else if(all(v(:,m)==(/0,0,0,0,0,0,0,0,1/))) then
                j2=maxloc(abs(C(m,:)))
